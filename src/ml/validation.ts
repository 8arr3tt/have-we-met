 ...
 */
export function isValidModelMetadata(metadata: unknown): metadata is ModelMetadata {
...
export function isValidRecordPair<T>(pair: unknown): pair is RecordPair<T> {
  ...
  return (
    p.record1 !== undefined &&
    p.record2 !== undefined &&
    (p.label === undefined || ['match', 'nonMatch', 'uncertain'].includes(p.label as string))
  );
}
 ...
 */
export function isValidMLPrediction(prediction: unknown): prediction is MLPrediction {
 ...
 */
export function isValidTrainingExample<T>(example: unknown): example is TrainingExample<T> {
 ...
 */
export function isValidTrainingDataset<T>(dataset: unknown): dataset is TrainingDataset<T> {
  ...
  return Array.isArray(d.examples) && d.examples.every((e) => isValidTrainingExample(e));
}

/**
 * Validate FieldFeatureConfig
 */
export function isValidFieldFeatureConfig(config: unknown): config is FieldFeatureConfig {
  if (!config || typeof config !== 'object') {
    return false;
  }
  ...
  return (
    ...
    (c.weight === undefined || (typeof c.weight === 'number' && c.weight > 0)) &&
    (c.includeMissingIndicator === undefined || typeof c.includeMissingIndicator === 'boolean')
  );
}
 ...
 */
export function isValidFeatureExtractionConfig(config: unknown): config is FeatureExtractionConfig {
  if (!config || typeof config !== 'object') {
    return false;
  }
 ...
 */
export function isValidMLIntegrationConfig(config: unknown): config is MLIntegrationConfig {
  if (!config || typeof config !== 'object') {
    return false;
  }
...
export function validateMLModelConfig(config: unknown): ValidationResult {
  ...
  if (typeof c.matchThreshold !== 'number') {
    errors.push({ field: 'matchThreshold', message: 'Must be a number', value: c.matchThreshold });
  ...
  if (
    ...
    errors.push({
      ...
      value: { matchThreshold: c.matchThreshold, nonMatchThreshold: c.nonMatchThreshold },
    });
  }
  ...
  if (typeof c.batchSize !== 'number') {
    errors.push({ field: 'batchSize', message: 'Must be a number', value: c.batchSize });
...
export function validateTrainingConfig(config: unknown): ValidationResult {
  ...
  if (typeof c.learningRate !== 'number') {
    errors.push({ field: 'learningRate', message: 'Must be a number', value: c.learningRate });
  ...
  if (typeof c.maxIterations !== 'number') {
    errors.push({ field: 'maxIterations', message: 'Must be a number', value: c.maxIterations });
  ...
  if (typeof c.regularization !== 'number') {
    errors.push({ field: 'regularization', message: 'Must be a number', value: c.regularization });
  ...
  } else if (c.earlyStoppingPatience <= 0 || !Number.isInteger(c.earlyStoppingPatience)) {
    errors.push({
      field: 'earlyStoppingPatience',
      message: 'Must be a positive integer',
      value: c.earlyStoppingPatience,
  ...
  if (typeof c.minImprovement !== 'number') {
    errors.push({ field: 'minImprovement', message: 'Must be a number', value: c.minImprovement });
