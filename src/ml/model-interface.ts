...
export abstract class BaseMLModel<T> implements MLModel<T> {
  ...
  async predictBatch(pairs: RecordPair<T>[]): Promise<BatchMLPrediction<T>[]> {
    ...
    for (let i = 0; i < pairs.length; i += batchSize) {
      ...
      const predictions = await Promise.all(batch.map((pair) => this.predict(pair)));
  ...
  protected calculateConfidence(probability: number): number {
    ...
    const midpoint = (this.config.matchThreshold + this.config.nonMatchThreshold) / 2;
    const halfRange = (this.config.matchThreshold - this.config.nonMatchThreshold) / 2;
