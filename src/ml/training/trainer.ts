...
export class ModelTrainer<T = Record<string, unknown>> {
  ...
  async train(dataset: TrainingDataset<T>): Promise<TrainingResult> {
    ...
    if (!this.featureExtractor) {
      return {
        ...
        earlyStopped: false,
        error:
          'Feature extractor not configured. Call setFeatureExtractor() first.',
      }
    }
   ...
   */
  private splitData(examples: TrainingExample<T>[]): {
    trainData: TrainingExample<T>[]
    valData: TrainingExample<T>[]
  } {
   ...
   */
  private initializeWeights(featureCount: number): {
    weights: number[]
    bias: number
  } {
  ...
  private computeGradients(
    ...
    for (let i = 0; i < n; i++) {
      ...
      totalLoss +=
        -y * Math.log(clampedPred) - (1 - y) * Math.log(1 - clampedPred)
  ...
  private shuffle<U>(array: U[]): U[] {
    const rng =
      this.seed !== undefined ? seededRandom(this.seed + 1) : Math.random
