...
import type { TrainingExample, TrainingDataset, RecordPair } from '../types'
...
> {
  ...
  collectFromQueueItem(
    ...
    const {
      decision,
      candidateRecord: queueCandidate,
      potentialMatches,
    } = queueItem
    ...
    if (decision.action === 'confirm' || decision.action === 'merge') {
      ...
      for (const pm of potentialMatches || []) {
        ...
        const feedbackItem = this.addFeedback(
          pair,
          label,
          source,
          {
...
export function createFeedbackCollector<
  T extends Record<string, unknown> = Record<string, unknown>,
>(options?: FeedbackCollectorOptions): FeedbackCollector<T> {
 ...
 */
export function queueDecisionToTrainingExample<
  T extends Record<string, unknown>,
>(queueItem: QueueItem<T>): TrainingExample<T> | null {
 ...
 */
export function queueDecisionsToTrainingExamples<
  T extends Record<string, unknown>,
>(queueItems: QueueItem<T>[]): TrainingExample<T>[] {
  const collector = new FeedbackCollector<T>()
