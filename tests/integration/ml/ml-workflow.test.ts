...
describe('ML Workflow Integration', () => {
  describe('Pre-trained Classifier Workflow', () => {
    ...
    it('should handle clearly different records', async () => {
      ...
      expect(prediction.probability).toBeLessThan(
        identicalPrediction.probability
      )
    })
  })
  ...
  describe('MLMatchIntegrator Workflow', () => {
    ...
    it('should support ML-only matching', async () => {
      ...
      const result = await integrator.matchWithMLOnly(candidate, existing, {
        noMatch: 30,
        definiteMatch: 65,
      })
      ...
      expect(result.score.totalScore).toBe(
        result.mlPrediction!.probability * 100
      )
    })
  ...
  describe('Batch Predictions', () => {
    it('should process multiple pairs efficiently', async () => {
      ...
      const pairs: RecordPair<Customer>[] = Array.from(
        { length: 10 },
        (_, i) => ({
          record1: {
            firstName: `Person${i}`,
