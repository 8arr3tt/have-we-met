...
import type { QueueItem, QueueStatus, PotentialMatch } from '../../../src/queue/types';
...
describe('Feedback Loop Integration', () => {
  describe('FeedbackCollector', () => {
    ...
    it('should collect feedback from queue confirm decision', () => {
      const queueItem = createMockQueueItem(
        ...
        [
          { record: { firstName: 'John', lastName: 'Smith', email: 'john@example.com' }, score: 85 },
        ],
        'confirm'
      );
    ...
    it('should filter feedback by criteria', () => {
      ...
      collector.addFeedback(
        { record1: { firstName: 'A', lastName: 'Test' }, record2: { firstName: 'A', lastName: 'Test' } },
        'match',
        'manual',
        { confidence: 0.95, matchScore: 0.9 }
      );
      collector.addFeedback(
        { record1: { firstName: 'B', lastName: 'Test' }, record2: { firstName: 'C', lastName: 'Other' } },
        'nonMatch',
        'manual',
        { confidence: 0.6, matchScore: 0.3 }
      );
      collector.addFeedback(
        { record1: { firstName: 'D', lastName: 'Test' }, record2: { firstName: 'E', lastName: 'Other' } },
        'nonMatch',
        'synthetic',
    ...
    it('should export balanced dataset', () => {
      // Add unbalanced data
      collector.addFeedback(
        { record1: { firstName: 'A1', lastName: 'Test' }, record2: { firstName: 'A1', lastName: 'Test' } },
        'match'
      );
      collector.addFeedback(
        { record1: { firstName: 'A2', lastName: 'Test' }, record2: { firstName: 'A2', lastName: 'Test' } },
        'match'
      );
      collector.addFeedback(
        { record1: { firstName: 'A3', lastName: 'Test' }, record2: { firstName: 'A3', lastName: 'Test' } },
        'match'
      );
      collector.addFeedback(
        { record1: { firstName: 'B', lastName: 'Test' }, record2: { firstName: 'C', lastName: 'Other' } },
      ...
      const balancedDataset = collector.exportAsTrainingDataset({ balance: true, seed: 42 });
  ...
  describe('Training Dataset Operations', () => {
    ...
    it('should calculate dataset statistics', () => {
      const examples: TrainingExample<Customer>[] = [
        ...
        createTrainingExample(
          { record1: { firstName: 'B', lastName: 'Test' }, record2: { firstName: 'B', lastName: 'Test' } },
          'match'
        ),
        createTrainingExample(
          { record1: { firstName: 'C', lastName: 'Test' }, record2: { firstName: 'D', lastName: 'Other' } },
          'nonMatch'
        ),
      ];
  ...
  describe('ModelTrainer', () => {
    ...
    it('should train model from labeled data', async () => {
      const examples: TrainingExample<Customer>[] = [
        // Matches
        createTrainingExample(
          { record1: { firstName: 'John', lastName: 'Smith' }, record2: { firstName: 'John', lastName: 'Smith' } },
          'match'
        ),
        createTrainingExample(
          { record1: { firstName: 'Jane', lastName: 'Doe' }, record2: { firstName: 'Jane', lastName: 'Doe' } },
          'match'
        ),
        // Non-matches
        createTrainingExample(
          { record1: { firstName: 'John', lastName: 'Smith' }, record2: { firstName: 'Jane', lastName: 'Doe' } },
          'nonMatch'
        ),
        createTrainingExample(
          { record1: { firstName: 'Bob', lastName: 'Wilson' }, record2: { firstName: 'Alice', lastName: 'Brown' } },
          'nonMatch'
        ),
      ];
    ...
    it('should support validation split and early stopping', async () => {
      const examples: TrainingExample<Customer>[] = Array.from({ length: 20 }, (_, i) => {
        const isMatch = i % 2 === 0;
    ...
    it('should report training progress', async () => {
      const examples: TrainingExample<Customer>[] = [
        ...
        createTrainingExample(
          { record1: { firstName: 'Jane', lastName: 'Doe' }, record2: { firstName: 'Bob', lastName: 'Wilson' } },
          'nonMatch'
        ),
      ];
  ...
  describe('Complete Feedback Loop', () => {
    it('should improve model with feedback data', async () => {
      ...
      const { classifier: initialClassifier } = await trainer.trainClassifier(initialDataset);
      ...
      collector.addFeedback(
        { record1: { firstName: 'Robert', lastName: 'Johnson' }, record2: { firstName: 'Bob', lastName: 'Johnson' } },
      ...
      collector.addFeedback(
        { record1: { firstName: 'Elizabeth', lastName: 'Williams' }, record2: { firstName: 'Liz', lastName: 'Williams' } },
        'match', // Human says these are the same (nickname)
        'manual',
        { confidence: 0.95 }
      );
      ...
      const combinedDataset = mergeTrainingDatasets(initialDataset, feedbackDataset);

      // Step 5: Retrain model
      const { classifier: improvedClassifier } = await trainer.trainClassifier(combinedDataset);
    ...
    it('should handle the full workflow from queue to training', async () => {
      ...
      const queueItems: QueueItem<Customer>[] = [
        createMockQueueItem(
          { firstName: 'John', lastName: 'Smith', email: 'john@example.com' },
          [{ record: { firstName: 'John', lastName: 'Smith', email: 'john@example.com' }, score: 95 }],
          'confirm'
        ),
        createMockQueueItem(
          { firstName: 'Jane', lastName: 'Doe', email: 'jane@a.com' },
          [{ record: { firstName: 'Janet', lastName: 'Doe', email: 'janet@b.com' }, score: 55 }],
          'reject'
        ),
        createMockQueueItem(
          { firstName: 'Bob', lastName: 'Wilson', email: 'bob@test.org' },
          [{ record: { firstName: 'Robert', lastName: 'Wilson', email: 'bob@test.org' }, score: 72 }],
          'merge'
        ),
      ];
