...
describe('ML Matching Integration', () => {
  ...
  describe('Resolver with ML', () => {
    ...
    it('should update ML configuration', async () => {
      const resolver = HaveWeMet.create<Person>()
        .schema((s) =>
          s.field('firstName').type('string').field('lastName').type('string')
        )
        .matching((match) =>
          match
            .field('firstName')
    ...
    it('should throw error when using ML methods without configuration', async () => {
      ...
      await expect(
        resolver.resolveMLOnly(
          { id: '1', firstName: 'John', lastName: 'Doe' },
          records
        )
      ).rejects.toThrow('ML is not configured')

      expect(() => resolver.extractMLFeatures(records[0], records[1])).toThrow(
        'ML is not configured'
      )
    })
  })
    ...
    it('should use fallback mode for uncertain results only', async () => {
      ...
      results.forEach((r) => {
        ...
        if (originalPotentialMatches.has(recordId)) {
          ...
          originalDefiniteMatches.has(recordId) ||
          originalNoMatches.has(recordId)
        ) {
  ...
  describe('Feature extraction', () => {
    it('should extract meaningful features from record pairs', async () => {
      ...
      const firstNameJWIndex = partialMatchFeatures.names.indexOf(
        'firstName_jaroWinkler'
      )
      if (firstNameJWIndex >= 0) {
        expect(partialMatchFeatures.values[firstNameJWIndex]).toBeGreaterThan(
          0.8
        )
      }
    })
  ...
  describe('ML Result Explanation', () => {
    ...
    it('should include feature importance in ML predictions', async () => {
      ...
      expect(results[0].mlPrediction?.featureImportance.length).toBeGreaterThan(
        0
      )
