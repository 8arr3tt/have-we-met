...
describe('SimpleClassifier', () => {
  ...
  describe('constructor', () => {
    ...
    it('should create with feature extractor', () => {
      expect(classifier.getMetadata().featureNames).toEqual(featureExtractor.getFeatureNames());
    });
  });
  ...
  describe('setFeatureExtractor', () => {
    it('should update feature extractor', () => {
      ...
      expect(c.getMetadata().featureNames).toEqual(featureExtractor.getFeatureNames());
    });
  });
  ...
  describe('extractFeatures', () => {
    it('should extract features from record pair', () => {
      const pair: RecordPair<TestRecord> = {
        ...
        record2: { firstName: 'Jon', lastName: 'Smith', email: 'john@example.com' },
      };
    ...
    it('should throw if feature extractor not set', () => {
      ...
      expect(() => c.extractFeatures(pair)).toThrow(/Feature extractor not configured/);
    });
  });
  ...
  describe('predict', () => {
    ...
    it('should predict non-match for dissimilar records', async () => {
      ...
      const pair: RecordPair<TestRecord> = {
        record1: { firstName: 'John', lastName: 'Smith', email: 'john@example.com' },
        record2: { firstName: 'Jane', lastName: 'Doe', email: 'jane@different.com' },
      };
  ...
  describe('predictBatch', () => {
    ...
    it('should predict batch of record pairs', async () => {
      const pairs: RecordPair<TestRecord>[] = [
        ...
        {
          record1: { firstName: 'Jane', lastName: 'Doe', email: 'jane@example.com' },
          record2: { firstName: 'Jane', lastName: 'Doe', email: 'jane@example.com' },
        },
      ];
  ...
  describe('updateWeights', () => {
    it('should update weights based on gradients', () => {
      ...
      for (let i = 0; i < initialWeights.length; i++) {
        expect(updatedWeights[i]).toBeCloseTo(initialWeights[i] - learningRate * gradients[i]);
      }
  ...
  describe('getFeatureCount', () => {
    it('should return feature extractor feature count', () => {
      expect(classifier.getFeatureCount()).toBe(featureExtractor.getFeatureCount());
    });
  ...
  describe('classification thresholds', () => {
    ...
    it('should classify as uncertain between thresholds', async () => {
      ...
      const pair: RecordPair<TestRecord> = {
        record1: { firstName: 'John', lastName: 'Smith', email: 'john@example.com' },
        record2: { firstName: 'John', lastName: 'Doe', email: 'john@different.com' },
      };
...
describe('createPersonMatchingClassifier', () => {
  it('should create classifier with person matching features', () => {
    ...
    expect(classifier.getMetadata().featureNames.some((n) => n.includes('firstName'))).toBe(true);
    expect(classifier.getMetadata().featureNames.some((n) => n.includes('lastName'))).toBe(true);
    expect(classifier.getMetadata().featureNames.some((n) => n.includes('email'))).toBe(true);
  });
});

describe('createClassifierFromFields', () => {
  it('should create classifier from field list', () => {
    const classifier = createClassifierFromFields<TestRecord>(['firstName', 'lastName', 'email']);
    expect(classifier.getMetadata().featureNames.length).toBeGreaterThan(0);
    expect(classifier.getMetadata().featureNames.some((n) => n.includes('firstName'))).toBe(true);
    expect(classifier.getMetadata().featureNames.some((n) => n.includes('lastName'))).toBe(true);
    expect(classifier.getMetadata().featureNames.some((n) => n.includes('email'))).toBe(true);
