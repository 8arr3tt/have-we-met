...
describe('FeatureExtractor', () => {
  ...
  describe('getFeatureNames', () => {
    it('should return feature names for each extractor', () => {
      const config: FeatureExtractionConfig = {
        fields: [{ field: 'firstName', extractors: ['jaroWinkler', 'exact'] }],
        normalize: true,
      }
...
describe('FeatureExtractionConfigBuilder', () => {
  ...
  it('should support addStringField', () => {
    const config = featureConfig().addStringField('name').build()
  ...
  it('should support addExactField', () => {
    const config = featureConfig().addExactField('id').build()

    expect(config.fields[0].extractors).toEqual(['exact'])
  })

  it('should support addNumericField', () => {
    const config = featureConfig().addNumericField('age').build()
  ...
  it('should support addDateField', () => {
    const config = featureConfig().addDateField('birthDate').build()
  ...
  it('should support addNameField', () => {
    const config = featureConfig().addNameField('firstName').build()
  ...
  it('should support addCustomField', () => {
    ...
    const config = featureConfig().addCustomField('special', customFn).build()
...
describe('Utility Functions', () => {
  ...
  describe('calculateFeatureStats', () => {
    ...
    it('should return null for unknown feature', () => {
      const vectors: FeatureVector[] = [{ values: [0.8], names: ['a'] }]
...
describe('Integration Tests', () => {
  ...
  it('should perform well on batch extraction', () => {
    ...
    const pairs: RecordPair<Person>[] = Array.from(
      { length: 1000 },
      (_, i) => ({
        record1: {
          firstName: `John${i}`,
