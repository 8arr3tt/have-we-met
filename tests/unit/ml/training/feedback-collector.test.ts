...
describe('FeedbackCollector', () => {
  ...
  describe('addFeedback', () => {
    ...
    it('should store queue item ID when provided', () => {
      ...
      const item = collector.addFeedback(pair, 'match', 'queue-confirm', {}, 'queue-123');
  ...
  describe('collectFromQueueItem', () => {
    ...
    it('should use custom candidate record when provided', () => {
      const queueItem = createMockQueueItem();
      const customCandidate: TestRecord = { id: '99', name: 'Custom', email: 'custom@example.com' };

      const feedback = collector.collectFromQueueItem(queueItem, customCandidate);
  ...
  describe('getFeedback', () => {
    ...
    it('should filter by match score range', () => {
      const items = collector.getFeedback({ minMatchScore: 0.5, maxMatchScore: 0.9 });
      expect(items.length).toBe(1);
      expect(items[0].quality.matchScore).toBe(0.8);
    });

    it('should filter by source', () => {
      const items = collector.getFeedback({ sources: ['queue-confirm', 'queue-reject'] });
      expect(items.length).toBe(2);
    });
  ...
  describe('exportAsTrainingDataset', () => {
    ...
    it('should balance dataset when requested', () => {
      ...
      expect(unbalanced.examples.filter((e) => e.label === 'match').length).toBe(3);
      expect(unbalanced.examples.filter((e) => e.label === 'nonMatch').length).toBe(1);

      const balanced = collector.exportAsTrainingDataset({ balance: true, seed: 42 });
      expect(balanced.examples.filter((e) => e.label === 'match').length).toBe(1);
      expect(balanced.examples.filter((e) => e.label === 'nonMatch').length).toBe(1);
    });
