...
import { FeatureExtractor, featureConfig } from '../../../../src/ml/feature-extractor';
import type {
  TrainingDataset,
  TrainingExample,
  TrainingMetrics,
...
function generateTestData(matchCount: number, nonMatchCount: number, seed: number = 42): TrainingExample<TestRecord>[] {
      ...
      createTrainingExample(
        {
          ...
          record2: { firstName: firstName, lastName: lastName + (rng() > 0.5 ? '' : 'e') }, // Small variation
        },
        'match',
        'synthetic'
      )
      ...
      createTrainingExample(
        {
          ...
          record2: { firstName: `Bob${i + 100}`, lastName: `Williams${i + 100}` },
        },
        'nonMatch',
        'synthetic'
      )
...
describe('createTrainingDataset', () => {
  it('should create dataset with computed statistics', () => {
    const examples: TrainingExample<TestRecord>[] = [
      createTrainingExample({ record1: { firstName: 'A', lastName: 'B' }, record2: { firstName: 'A', lastName: 'B' } }, 'match'),
      createTrainingExample({ record1: { firstName: 'C', lastName: 'D' }, record2: { firstName: 'C', lastName: 'D' } }, 'match'),
      createTrainingExample({ record1: { firstName: 'E', lastName: 'F' }, record2: { firstName: 'X', lastName: 'Y' } }, 'nonMatch'),
    ];
...
describe('mergeTrainingDatasets', () => {
  it('should merge multiple datasets', () => {
    ...
    const dataset2 = createTrainingDataset<TestRecord>([
      createTrainingExample({ record1: { firstName: 'C', lastName: 'D' }, record2: { firstName: 'X', lastName: 'Y' } }, 'nonMatch'),
    ]);
...
describe('balanceDataset', () => {
  it('should balance imbalanced dataset by undersampling', () => {
    const examples: TrainingExample<TestRecord>[] = [
      ...Array.from({ length: 10 }, (_, i) =>
        createTrainingExample({ record1: { firstName: `M${i}`, lastName: 'X' }, record2: { firstName: `M${i}`, lastName: 'X' } }, 'match')
      ),
      ...Array.from({ length: 3 }, (_, i) =>
        createTrainingExample({ record1: { firstName: `N${i}`, lastName: 'A' }, record2: { firstName: `N${i}`, lastName: 'B' } }, 'nonMatch')
      ),
    ];
  ...
  it('should produce reproducible results with seed', () => {
    ...
    expect(balanced1.examples.map(e => e.pair.record1.firstName))
      .toEqual(balanced2.examples.map(e => e.pair.record1.firstName));
  });
});
...
describe('getDatasetStats', () => {
  it('should calculate correct statistics', () => {
    const examples: TrainingExample<TestRecord>[] = [
      ...
      createTrainingExample({ record1: { firstName: 'C', lastName: 'D' }, record2: { firstName: 'C', lastName: 'D' } }, 'match'),
      createTrainingExample({ record1: { firstName: 'E', lastName: 'F' }, record2: { firstName: 'X', lastName: 'Y' } }, 'nonMatch'),
      createTrainingExample({ record1: { firstName: 'G', lastName: 'H' }, record2: { firstName: 'Z', lastName: 'W' } }, 'nonMatch'),
    ];
  ...
  it('should detect imbalanced dataset', () => {
    const examples: TrainingExample<TestRecord>[] = [
      ...
      createTrainingExample({ record1: { firstName: 'X', lastName: 'Y' }, record2: { firstName: 'Z', lastName: 'W' } }, 'nonMatch'),
    ];
...
describe('exportWeightsToJson', () => {
  ...
  it('should throw on failed training result', () => {
    const failedResult = {
      success: false,
      finalMetrics: { iteration: 0, trainingLoss: Infinity, trainingAccuracy: 0 },
      history: [],
      trainingTimeMs: 0,
      earlyStopped: false,
      error: 'Test error',
