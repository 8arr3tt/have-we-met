...
describe('MLBuilder', () => {
  describe('basic configuration', () => {
    ...
    it('custom model overrides usePretrained', () => {
      ...
      const config = mlBuilder().usePretrained().model(mockModel).build()
  ...
  describe('integration mode configuration', () => {
    ...
    it('throws on invalid mlWeight', () => {
      expect(() => mlBuilder().mlWeight(-0.1)).toThrow(
        'ML weight must be between 0 and 1'
      )
      expect(() => mlBuilder().mlWeight(1.1)).toThrow(
        'ML weight must be between 0 and 1'
      )
    })
  ...
  describe('field feature configuration', () => {
    ...
    it('configures includeMissing', () => {
      ...
      expect(config.featureConfig?.fields[0].includeMissingIndicator).toBe(
        false
      )
    })
  })
  ...
  describe('bulk field configuration', () => {
    ...
    it('configures name fields', () => {
      const config = mlBuilder().nameFields(['firstName', 'lastName']).build()
    ...
    it('configures identifier fields', () => {
      const config = mlBuilder().identifierFields(['email', 'ssn']).build()
    ...
    it('configures date fields', () => {
      const config = mlBuilder().dateFields(['dob', 'createdAt']).build()
  ...
  describe('custom extractors', () => {
    it('adds custom extractor', () => {
      ...
      expect(config.featureConfig?.customExtractors?.['customField']).toBe(
        customFn
      )
    })
...
describe('validateMLBuilderConfig', () => {
  ...
  it('detects non-match threshold >= match threshold', () => {
    ...
    expect(errors).toContain(
      'Non-match threshold must be less than match threshold'
    )
  })
