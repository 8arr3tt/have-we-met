...
import { DEFAULT_ML_MODEL_CONFIG, DEFAULT_ML_INTEGRATION_CONFIG } from '../../../../src/ml/types';
import type { MatchResult, MatchScore } from '../../../../src/core/scoring/types';
...
class MockMLModel<T> implements MLModel<T> {
  ...
  private ready = true;
  public predictFn: (pair: RecordPair<T>) => Promise<MLPrediction> = async () => ({
    probability: 0.8,
  ...
  async predictBatch(pairs: RecordPair<T>[]): Promise<Array<{ pair: RecordPair<T>; prediction: MLPrediction }>> {
    const results = [];
...
describe('MLMatchIntegrator', () => {
  ...
  describe('constructor', () => {
    it('should create integrator with default config', () => {
      ...
      expect(config.fallbackOnError).toBe(DEFAULT_ML_INTEGRATION_CONFIG.fallbackOnError);
    });
  ...
  describe('enhanceMatchResult', () => {
    ...
    it('should only apply ML to uncertain cases when applyTo is uncertainOnly', async () => {
      ...
      const definiteResult = createMockMatchResult(existingRecord, 80, 'definite-match');
      ...
      const potentialResult = createMockMatchResult(existingRecord, 50, 'potential-match');
  ...
  describe('enhanceMatchResults', () => {
    ...
    it('should skip all ML when skipML is true', async () => {
      ...
      const probabilisticResults = [createMockMatchResult(existingRecords[0], 50)];
