...
describe('Performance: ML Predictions', () => {
  it('should complete ML prediction in <10ms', async () => {
    ...
    console.log(`ML prediction avg: ${avgDuration.toFixed(2)}ms, p95: ${p95Duration.toFixed(2)}ms`)
  })
  ...
  it.skip('should maintain ML prediction consistency', async () => {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      if (result.outcome === 'match' && result.matches[0]?.mlScore !== undefined) {
  ...
  it.skip('should not degrade with repeated ML predictions', async () => {
    ...
    const firstAvg = firstBatchDurations.reduce((a, b) => a + b, 0) / firstBatchDurations.length
    const lastAvg = lastBatchDurations.reduce((a, b) => a + b, 0) / lastBatchDurations.length
    const degradation = ((lastAvg - firstAvg) / firstAvg) * 100

    expect(Math.abs(degradation)).toBeLessThan(20) // Less than 20% change
    console.log(`Performance change: ${degradation.toFixed(2)}% (first: ${firstAvg.toFixed(2)}ms, last: ${lastAvg.toFixed(2)}ms)`)
  })
})
